...
typedef enum {
    ...
    TLS_KEYEX_STATE_RENEGO_HRR   = 4, // negotiate again since the client receives the first HelloRetryRequest
...
typedef enum {
    ...
    // indicate that current piece of data (ready to send or already received) is the first fragment of record message
    TLS_RESP_FIRST_FRAG              =  4,
    // indicate that current piece of data (ready to send or already received) is the final fragment of record message
    ...
    TLS_RESP_ERR_NOT_SUPPORT         = -8, // features / functions defined in TLS protocol, but not supported in this implementation
    TLS_RESP_TIMEOUT                 = -9,
    TLS_RESP_ERR_NO_KEYEX_MTHD_AVAIL = -10,
    TLS_RESP_ERR_KEYGEN              = -11,
    TLS_RESP_ERR_HASH                = -12,
    TLS_RESP_ERR_ENCRYPT             = -13, // error on encryption / decryption functions (including initialization)
    ...
    TLS_RESP_ERR_EXCEED_MAX_REC_SZ   = -17, // error because received record message exceeds maximum size defined in this implementation
    ...
    TLS_RESP_ILLEGAL_PARAMS          = -22, // errors which conform to formal protocol syntax, but are incorrect or inconsistent
    TLS_RESP_ERR_SERVER_NAME         = -23, // errors for the sevrer which cannot find (another) appropriate server specified in
                                            // "server_name" extension entry of ClientHello
    TLS_RESP_REQ_ALERT               = -50, // errors found when decoding message from peer, then the client will send alert
} tlsRespStatus;
...
// [Note] no need to consider collision because this implementation ONLY supports few hash / public key algorithm
typedef enum {
...
typedef struct __tlsCert {
    ...
    // for temporarily storing raw bytes of CertificateEntry.cert_data<1..2^24-1> before decoding it,
    // once it's decoded, this implementation will free up the space from the rawbyte field.
    // Note this implementation restricts size of certificate chain (CertificateEntry.cert_data from peer)
    // , it should depend on RAM size of underlying hardware platform
...
typedef  struct { // structure particularly used at key exchange phase of handshaking process
    ...
    tlsKeyExState   *grp_nego_state; // flags to set when a named group is negotiated at the key exchange phase.
    byte             num_grps_total; // total number of items in the supported named group list
    byte             num_grps_chosen;  // number of items in the key share extension (in ClientHello), with respect to
                                       // the supported named groups
    byte             chosen_grp_idx;   // the index of grp_nego_state array item which is chosen at key-exchange phase
} tlsKeyEx_t;
...
typedef struct { // necessary elements for building secure connection
    struct { // the hashed handshake message so far, it will be examined when sending FINISHED handshake message.
        tlsHash_t    *objsha256;
        tlsHash_t    *objsha384;
    ...
    byte            *client_rand;
    byte            *server_rand;
    // the read / write key here are used in either handshaking process or application data encryption.
    // It seems impossible to use both.
    ...
    // in TLS v1.3, per-record nonce (see section 5.3) is derived by 64-bit value (incremented by one
    // for each decrypting / encrypting record message) and IV derived by previous HKDF procedure.
    ...
    struct {
        // indicate that currently processing block is the first fragment of TLSciphertext (if split to multiple packets to transmit)
        byte   ct_first_frag:1;
        // indicate that currently processing block is the final fragment of TLSciphertext, which must contain authentication tag
...
typedef struct {
    ...
    word16          inlen_decrypted;   // number of bytes already received, decrypted from inbuf, but haven't been decoded yet
    word16          inlen_unprocessed; // number of bytes already received from peer, but haven't been decrypted/decoded yet in inbuf
    word16          inlen_total;       // total number of bytes for current in-flight record message
    word16          outlen_encoded;    // number of bytes already encoded to outbuf, but haven't been encrypted & sent out yet
    word16          outlen_encrypted;  // number of bytes already encoded & encrypted to outbuf, but haven't been sent out yet
    ...
    // used to store number of copied bytes for a decoding/encoding extension entry, if the entire entry cannot
    // be copied to current fragment of the in-flight/out-flight message & has to be split to multiple fragments.
    // The MSB of last_ext_entry_xxx_len is used to indicate whether it stores copied bytes for :
    // (1) total size of the given extension list, if bit 15 of last_ext_entry_xxx_len is set
    // (2) currently processing extension entry,   if bit 15 is clear
    ...
    union {
        ...
        word32       remaining_to_send; // used for sending fragments of client's Certificate, CertificateVerify, Finished
    ...
    tlsOpaque16b_t   client_signed_sig;  // signed signature that is ready to send with client's CertificateVerify message
    union {
        tlsOpaque8b_t    session_id;    // preserve session ID (max #bytes : 32) during ClientHello, ServerHello
        tlsOpaque8b_t    cert_req_ctx;  // preserve certificate_request_context from server's CertificateRequest
        tlsOpaque8b_t    finish_verifydata; // verify data that is ready to send with client's Finished message
    } tmpbuf; // temperary buffer pointer used in the TLS session

    // if current encoding/decoding record message is too large to fit in one single TCP packet, then we split it 
    // to several fragments (smaller packets), and set this flag after first fragment was sent/received.
    // (the first fragment of a record message always contains record header)
    ...
    struct { // temporarily store result since its last operation
        ...
        // In this MQTT implementation, it is unlikely to send / receive over 256 encrypted record messages for
        // each established TLS session, so only 8-bit counting numbers are applied, which represent number of
        // write (sent) / read (received) encrypted record messages accordingly. These values can be ingredients
        // of deriving nonce to AEAD encryption / decryption function.
    ...
    struct {
        // it is set whenever the peer (server) reply with ClientHelloRetry to this client, clear if client receives ServerHello
        ...
        byte     hs_server_finish:1; // set after Finished message received from server is successfully decoded.
        byte     hs_client_finish:1; // set after Finished message on client side is successfully sent to the peer
