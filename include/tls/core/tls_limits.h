...
#define REPORT_MAXVAL_ERROR(variable, maxval) \
    "[ERROR] " #variable " must be smaller than default value " #maxval \
    " defined in this implementation. Recheck your configuration."
...
// number of bytes in record layer must NOT exceed 8 KBytes - 1
    ...
    #endif
#else
    // default = 0xffff, but users can limit the packet size for resource-constraint embedded
    // devices
    #define TLS_MAX_BYTES_RECORD_LAYER_PKT TLS_DEFAULT_BYTES_RECORD_LAYER_PKT
#endif // end of TLS_MAX_BYTES_RECORD_LAYER_PKT

#define TLS_DEFAULT_BYTES_HANDSHAKE_MSG \
    (TLS_MAX_BYTES_RECORD_LAYER_PKT - TLS_RECORD_LAYER_HEADER_NBYTES)
...
// maximum number of bytes that represent a list of supported cipher suites in ClientHello /
...
#define TLS_DEFAULT_BYTES_CERT_CHAIN \
    (TLS_MAX_BYTES_HANDSHAKE_MSG - TLS_HANDSHAKE_HEADER_NBYTES - 1 - \
     TLS_MAX_BYTES_HS_CERT_REQ_CTX - 3)
...
#define TLS_MAX_BYTES_HKDF_EXPAND_LABEL \
    (TLS_MAX_BYTES_HKDF_EXPAND_INFO - (sizeof(TLS_HKDF_LABEL_PREFIX) - 1) - 1 - 1 - 2)
