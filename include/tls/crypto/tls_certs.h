...
// Note final_item_rdy indicates the entire cert chain is ready to decode, or fianl cert item is NOT
// ready, it is used when multiple fragments of the cert chain are transmitting from the peer
...
// NOT supported .
tlsRespStatus tlsCopyCertRawData(tlsSession_t *);
...
tlsRespStatus tlsSignCertSignature(
    void *privkey, mqttDRBG_t *, tlsOpaque16b_t *in, tlsOpaque16b_t *out, tlsAlgoOID sign_algo,
    tlsRSApss_t *rsapssextra
);
...
// * Context string, for CertificateVerify from server, it should be "TLS 1.3, server
// CertificateVerify"
...
tlsRespStatus tlsCertVerifyGenDigitalSig(
    tlsSecurityElements_t *, const tlsRSApss_t *rsapss_attri, tlsOpaque16b_t *out,
