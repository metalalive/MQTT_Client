...
typedef enum {
    MQTT_RESP_OK = 0,
    MQTT_RESP_OK_IGNOREMORE = 0, // Function succedded, but ignore subsequent payload bytes or
                                 // properties to send / receive.
    ...
    MQTT_RESP_ERRMEM = -6,  // Memory error occurred, e.g. memory leak, requesting space exceeds
                            // limit of the system platform
    MQTT_RESP_TIMEOUT = -7, // Timeout on network connection, or control packet transmission, or no
                            // response from underlying system
    MQTT_RESP_INPROG =
        -8, // MQTT packet incoming / outgoing transmission hasn't been finished, still in progress.
    ...
    MQTT_RESP_ERR_EXCEED_PKT_SZ = -11, // error occurs if the receiving / sending packet size
                                       // exceeds the limit in system platform or protocol (256MB)
    ...
    MQTT_RESP_ERR_PROP_REPEAT = -18, // duplicate property present in a give property list (for few
                                     // properties it doesn't matter)
    MQTT_RESP_ERR_INTEGRITY = -19,   // integrity error, used when a given C struct variable lacks
                                     // some data which must be prepared.
    MQTT_RESP_INVALID_TOPIC = -20,   // error occurs when a topic string is incorrectly formed in
                                     // SUBSCRIBE or PUBLISH packet
} mqttRespStatus;
...
typedef struct {
    mqttDrbgHash_t hash;
    mqttStr_t      entropy;
    // once user calls PRNG function: mqttUtilPRNG() running the entire DRBG algorithm, and
    // generates random byte sequence. the generated byte sequence can be preserved as cache in case
    // the current function call does ONLY need small portion of the generated byte sequence, this
    // means part of the generated byte sequence can be used for next few PRNG function calls. Also
    // when the all bytes of cache are read once, users MUST run the entire DRBG algorithm again
    mqttStr_t cache;
