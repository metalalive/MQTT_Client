...
UART_HandleTypeDef *STM32_config_UART(void) {
    return &haluart3;
}
DMA_HandleTypeDef  *STM32_config_DMA4UART(void) {
    return &haldma_usart3_rx;
}

void USART3_IRQHandler( void ) {
    STM32_generic_USART_IRQHandler(&haluart3);
}

void DMA1_Stream1_IRQHandler(void) {
    STM32_generic_DMAstream_IRQHandler(&haldma_usart3_rx);
}
