...
static mqttPropertyType connectPropTypeList[] = {
    MQTT_PROP_SESSION_EXPIRY_INTVL, MQTT_PROP_RECV_MAX,      MQTT_PROP_MAX_PKT_SIZE,
    MQTT_PROP_TOPIC_ALIAS_MAX,      MQTT_PROP_REQ_RESP_INFO, MQTT_PROP_REQ_PROBLEM_INFO,
    MQTT_PROP_USER_PROPERTY,        MQTT_PROP_USER_PROPERTY,
    //// MQTT_PROP_AUTH_METHOD     , // TODO: implement & test enhanced authentication, which is
    /// optional. / MQTT_PROP_AUTH_DATA       ,
};
...
static const mqttStr_t mqttTestTopicLevelName[MQTT_TEST_TOPIC_LVL_NAMES_ARRAY_LENGTH]
                                             [MQTT_TEST_TOPIC_LVL_NAMES_ARRAY_LENGTH] = {
...
mqttTestRandGenSubsTopics(mqttDRBG_t *drbg, mqttTopic_t **topics, word16 topic_cnt) {
    mqttRespStatus status = MQTT_RESP_OK;
    word32         idx = 0;
    mqttTopic_t   *topics_p = NULL;

    if (topic_cnt == 0 || topics == NULL) {
        return MQTT_RESP_ERRARGS;
    }
...
static void mqttTestRandSetupProp(mqttDRBG_t *drbg, mqttProp_t *curr_prop) {
    ...
    case MQTT_PROP_MAX_PKT_SIZE:
        curr_prop->body.u32 =
            (MQTT_RECV_PKT_MAXBYTES >> 1) + mqttUtilPRNG(drbg, MQTT_RECV_PKT_MAXBYTES >> 2);
        break;
...
static mqttRespStatus mqttTestRandSetupProps(
    mqttDRBG_t *drbg, mqttPropertyType *given_arr, size_t list_size, mqttProp_t **head_prop
) {
    uint8_t          num_props = 0;
    ...
    for (idx = 0; idx < num_props; idx++) {
        ...
        // so the available items become given_arr[0] ... given_arr[list_size - 2] in next
        // iteration.
...
static mqttRespStatus mqttTestGenPattConnect(mqttDRBG_t *drbg, mqttConn_t *mconn) {
    ...
    status = mqttTestRandSetupProps(
        drbg, (mqttPropertyType *)&connectPropTypeList, XGETARRAYSIZE(connectPropTypeList),
        &mconn->props
    );
    if (status < 0) {
        return status;
    ...
    if (mconn->flgs.will_enable == 1) {
        ...
        status = mqttTestRandSetupProps(
            drbg, (mqttPropertyType *)&willPropTypeList, XGETARRAYSIZE(willPropTypeList),
            &lwtmsg->props
        );
        if (status < 0) {
            return status;
        ...
        mqttTestRandGenStr(
            drbg, &lwt_payld_data[default_lwt_str_len], (lwt_payld_len - default_lwt_str_len)
        );
...
static mqttRespStatus
mqttTestGenPattPublish(mqttDRBG_t *drbg, mqttMsg_t *pubmsg, word32 send_pkt_maxbytes) {
    mqttRespStatus status = MQTT_RESP_OK;
    ...
    status = mqttTestRandSetupProps(
        drbg, (mqttPropertyType *)&publishPropTypeList, XGETARRAYSIZE(publishPropTypeList),
        &pubmsg->props
    );
    if (status < 0) {
        return status;
...
static mqttRespStatus mqttTestGenPattSubscribe(mqttDRBG_t *drbg, mqttPktSubs_t *subs) {
    ...
    status = mqttTestRandSetupProps(
        drbg, (mqttPropertyType *)&subscribePropTypeList, XGETARRAYSIZE(subscribePropTypeList),
        &subs->props
    );
    if (status < 0) {
        return status;
...
static mqttRespStatus mqttTestGenPattUnsubscribe(
    mqttDRBG_t *drbg, mqttPktUnsubs_t *unsubs_out, const mqttPktSubs_t *subs_in
) {
    mqttRespStatus status = MQTT_RESP_OK;
    ...
    status = mqttTestRandSetupProps(
        drbg, (mqttPropertyType *)&unsubscribePropTypeList, XGETARRAYSIZE(unsubscribePropTypeList),
        &unsubs_out->props
    );
    if (status < 0) {
        return status;
    ...
    for (idx = 0; idx < unsubs_out->topic_cnt; idx++) {
        unsubs_out->topics[idx].filter.data =
            (byte *)XMALLOC(sizeof(byte) * subs_in->topics[idx].filter.len);
        if (unsubs_out->topics[idx].filter.data == NULL) {
            return MQTT_RESP_ERRMEM;
        }
        XMEMCPY(
            unsubs_out->topics[idx].filter.data, subs_in->topics[idx].filter.data,
            subs_in->topics[idx].filter.len
        );
    }
    return status;
...
static mqttRespStatus mqttTestGenPattDisconnect(mqttDRBG_t *drbg, mqttPktDisconn_t *disconn) {
    ...
    status = mqttTestRandSetupProps(
        drbg, (mqttPropertyType *)&disconnPropTypeList, XGETARRAYSIZE(disconnPropTypeList),
        &disconn->props
    );
    if (status < 0) {
        return status;
...
mqttRespStatus
mqttTestCopyPatterns(mqttTestPatt *patt_in, mqttCtx_t *mctx, mqttCtrlPktType cmdtype) {
    mqttRespStatus status = MQTT_RESP_OK;
    ...
    case MQTT_PACKET_TYPE_DISCONNECT:
        ...
        if (status == MQTT_RESP_OK) {
            XMEMCPY(
                (void *)&mctx->send_pkt.disconn, (void *)&patt_in->disconn, sizeof(mqttPktDisconn_t)
            );
        }
        break;
    case MQTT_PACKET_TYPE_PUBLISH:
        patt_in->send_pkt_maxbytes = mctx->send_pkt_maxbytes;
        status = mqttTestGenPattPublish(
            patt_in->drbg, &patt_in->pubmsg_send, patt_in->send_pkt_maxbytes
        );
        if (status == MQTT_RESP_OK) {
            XMEMCPY(
                (void *)&mctx->send_pkt.pub_msg, (void *)&patt_in->pubmsg_send, sizeof(mqttMsg_t)
            );
        }
        break;
    ...
    case MQTT_PACKET_TYPE_UNSUBSCRIBE:
        ...
        if (status == MQTT_RESP_OK) {
            XMEMCPY(
                (void *)&mctx->send_pkt.unsubs, (void *)&patt_in->unsubs, sizeof(mqttPktUnsubs_t)
            );
        }
        break;
...
mqttRespStatus mqttTestCleanupPatterns(mqttTestPatt *patt_in, mqttCtrlPktType cmdtype) {
    ...
    case MQTT_PACKET_TYPE_CONNECT: {
        ...
        if (conn->flgs.will_enable ==
            1) { // TODO: build API to free memory allocated to the mqttMsg_t packet
