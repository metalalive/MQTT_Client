...
// record number of bytes encrypted in the last encrypt/decrypt function call, excluding length of
// auth tag
...
TEST_GROUP_RUNNER(tlsEncryptRecordMsg) { RUN_TEST_CASE(tlsEncryptRecordMsg, one_frag_multi_msgs); }

TEST_GROUP_RUNNER(tlsDecryptRecordMsg) { RUN_TEST_CASE(tlsDecryptRecordMsg, one_msg_multi_frags); }
...
TEST(tlsEncryptRecordMsg, one_frag_multi_msgs) {
    ...
    len = tls_session->curr_outmsg_len - TLS_RECORD_LAYER_HEADER_NBYTES -
    ...
    tls_session->curr_outmsg_len =
        (tls_session->outbuf.len >> 1) + tls_session->sec.chosen_ciphersuite->tagSize;
    tls_session->outlen_encoded +=
        (tls_session->outbuf.len >> 1); // assume all data bytes are encoded except auth tag
    ...
    buf =
        tls_session->outbuf.data + tls_session->curr_outmsg_start + TLS_RECORD_LAYER_HEADER_NBYTES;
    len = tls_session->outlen_encrypted - tls_session->curr_outmsg_start -
          TLS_RECORD_LAYER_HEADER_NBYTES;
    TEST_ASSERT_EQUAL_UINT(buf, mock_encrypt_data_start);
    TEST_ASSERT_EQUAL_UINT32(len, mock_encrypt_len);
    // calculate the bytes encoded but not encrypted in current fragment.
    len = ((tls_session->outbuf.len >> 1) - TLS_RECORD_LAYER_HEADER_NBYTES) %
          tls_session->sec.chosen_ciphersuite->tagSize; //
    TEST_ASSERT_EQUAL_UINT16(tls_session->outlen_encoded, (tls_session->outlen_encrypted + len));

    // rest of data bytes in the second record message, previously encoded, and encrypted, sent in
    // this fragment.
    tls_session->remain_frags_out = 1;
    tls_session->num_frags_out = 2;
    tls_session->curr_outmsg_start = 0;
    // "len" conatains number of the data bytes previously encoded, but not encrypted, sent in
    // previous fragment.
...
TEST(tlsDecryptRecordMsg, one_msg_multi_frags) {
    ...
    len = (tls_session->inbuf.len - TLS_RECORD_LAYER_HEADER_NBYTES) %
          tls_session->sec.chosen_ciphersuite->tagSize;
    // the first fragment
    ...
    tls_session->inlen_total =
        tls_session->inbuf.len - len - 2 + tls_session->sec.chosen_ciphersuite->tagSize;
    ...
    // the second fragment, part of auth tag is received, but cannot be used yet for authentication
    // encryption because it is not complete.
    tls_session->remain_frags_in = 2;
    ...
    TEST_ASSERT_EQUAL_UINT16(
        (tls_session->inbuf.len - tls_session->sec.chosen_ciphersuite->tagSize),
    ...
    TEST_ASSERT_EQUAL_UINT16(
        (tls_session->sec.chosen_ciphersuite->tagSize * 2 - 2), tls_session->inlen_decrypted
    );
