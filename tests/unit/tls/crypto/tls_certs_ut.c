...
static byte mock_cert_hashed_dn[NUM_PEER_CERTS + NUM_CA_CERTS]
                               [NBYTES_CERT_HASHED_DISTINGUISHED_NAME];
...
tlsRespStatus tlsDecodeX509cert(tlsCert_t *cert) {
    ...
    if (cert->signature.data == NULL) {
        ...
        cert->sign_algo =
            mock_cert_sig_algo; // or TLS_ALGO_OID_RSASSA_PSS or TLS_ALGO_OID_SHA384_RSA_SIG
...
void tlsX509FreeCertExt(tlsX509v3ext_t *in) {} // end of tlsX509FreeCertExt

void tlsRSAfreePubKey(void *pubkey_p) { XMEMFREE(pubkey_p); } // end of tlsRSAfreePubKey
...
TEST_GROUP_RUNNER(tlsDecodeCerts) { RUN_TEST_CASE(tlsDecodeCerts, decode_multi_certs); }
...
TEST_GROUP_RUNNER(tlsSignCertSignature) { RUN_TEST_CASE(tlsSignCertSignature, rsa_pss); }
...
TEST(tlsCopyCertRawData, multi_certs_multi_frags) {
    ...
    tls_session->inlen_decoded =
        TLS_RECORD_LAYER_HEADER_NBYTES + TLS_HANDSHAKE_HEADER_NBYTES + 1 + 0 + 3;

    // ---------- generate cert chain bytes for test ----------
    certs_rawbyte_sz[0] =
        tls_session->inbuf.len - tls_session->inlen_decoded - 3 - 2 - certs_ext_sz[0] - 1;
    ...
    nbytes_certs_cpy = XMIN(
        certchain_sz - nbytes_certs_copied, tls_session->inbuf.len - tls_session->inlen_decoded
    );
    ...
    for (idx = 1; idx < 4; idx++) {
        ...
        XMEMCPY(
            &tls_session->inbuf.data[tls_session->inlen_decoded],
            &certchain_rawbytes[nbytes_certs_copied], nbytes_certs_cpy
        );
        nbytes_certs_copied += nbytes_certs_cpy;
        status = tlsCopyCertRawData(tls_session);
        TEST_ASSERT_EQUAL_INT(TLS_RESP_OK, status);
    ...
    for (idx = 4; idx < NUM_PEER_CERTS; idx++) {
        ...
        if (idx > 4) {
            ...
            TEST_ASSERT_EQUAL_UINT16(
                TLS_EXT_TYPE_SIGNED_CERTIFICATE_TIMESTAMP, curr_cert->exts->type
            );
    ...
    tls_session->inlen_decrypted =
        nbytes_certs_cpy + 1 + tls_session->sec.chosen_ciphersuite->tagSize;
    status = tlsCopyCertRawData(tls_session);
    TEST_ASSERT_EQUAL_INT(TLS_RESP_OK, status);
    TEST_ASSERT_NOT_EQUAL(NULL, curr_cert->exts);
...
TEST(tlsDecodeCerts, decode_multi_certs) {
    ...
    for (idx = 0, curr_cert = tls_session->peer_certs; idx < (NUM_PEER_CERTS >> 1);
         idx++, middle_cert = curr_cert, curr_cert = curr_cert->next)
        ;
    ...
    tlsFreeCertChain(
        middle_cert, TLS_FREE_CERT_ENTRY_RAWBYTE | TLS_FREE_CERT_ENTRY_SKIP_FINAL_ITEM
    );
    TEST_ASSERT_EQUAL_INT(TLS_RESP_OK, status);
    // the cert items prior to middle_cert shouldn't be modified
    for (curr_cert = tls_session->peer_certs; curr_cert != middle_cert;
         curr_cert = curr_cert->next) {
        TEST_ASSERT_NOT_EQUAL(NULL, curr_cert->rawbytes.data);
...
static void tlsVerifyCertChain_verify_single_chain(byte *cert_issuing_order) {
    ...
    cert_issuing_order[NUM_PEER_CERTS - 1] =
        0; // the final item must always be zero, that represents index of server cert
    //// printf("xxx: ");
    //// for(idx = 0; idx < NUM_PEER_CERTS; idx++) {
    ////     printf("%d, ", cert_issuing_order[idx]);
    //// } // end of for loop
    //// printf("\r\n");
    mock_cert_issuer_hashed_dn[cert_issuing_order[0]] =
        mock_cert_subject_hashed_dn[cert_issuing_order[0]];
    for (idx = 1; idx < NUM_PEER_CERTS; idx++) {
        mock_cert_issuer_hashed_dn[cert_issuing_order[idx]] =
            mock_cert_subject_hashed_dn[cert_issuing_order[idx - 1]];
    } // end of for loop
    for (idx = 0, curr_cert = tls_session->peer_certs; curr_cert != NULL;
        ...
        XMEMCPY(
            curr_cert->subject.hashed_dn, &mock_cert_subject_hashed_dn[idx],
            NBYTES_CERT_HASHED_DISTINGUISHED_NAME
        );
...
static void tlsVerifyCertChain_verify_all_possible_chains(
    word16 avail_num, word16 total_num, byte *combination_selected_flags, byte *cert_issuing_order
) {
    word16 idx = 0;
    if (avail_num == 0) {
    ...
    for (idx = 0; idx < total_num; idx++) {
        if (combination_selected_flags[idx] == 0) {
            ...
            tlsVerifyCertChain_verify_all_possible_chains(
                avail_num - 1, total_num, combination_selected_flags, cert_issuing_order
            );
...
TEST(tlsVerifyCertChain, without_issuer_cert) {
    ...
    tlsVerifyCertChain_verify_chains_wrapper(
        NUM_PEER_CERTS - 1, &combination_selected_flags[0], &cert_issuing_order[0]
    );
...
TEST(tlsCertVerifyGenDigitalSig, server_side) {
    ...
    status = tlsCertVerifyGenDigitalSig(
        &tls_session->sec, (const tlsRSApss_t *)&rsapssSig, &gened_digi_sig, is_server
    );
    TEST_ASSERT_EQUAL_INT(TLS_RESP_OK, status);
...
TEST(tlsSignCertSignature, rsa_pss) {
    ...
    tls_session->client_signed_sig.data =
        XMALLOC(sizeof(byte) * tls_session->client_signed_sig.len);
    digiSig.len = rsapssSig.salt_len;
    digiSig.data = XMALLOC(digiSig.len
