...
TEST(tlsHandshakeHash, tlsTransHashTakeSnapshot)
{
    ...
    mock_hash_curr_outbytes[sha384_idx] = (const byte *)&("It is supposed to be the hash output, 48 bytes in total (SHA384)");
    ...
    mock_hash_curr_outbytes[sha256_idx] = (const byte *)&("Bytes that are supposed to load, 32 bytes in total (SHA256)");
    ...
    TEST_ASSERT_EQUAL_STRING_LEN(expect_hash_state[sha256_idx], sec->hashed_hs_msg.objsha256, 5); // hash state must not be changed after snapshot
...
TEST(tlsTranscrptHashHSmsgUpdate, outbuf_one_frag_multi_msgs)
{
    ...
    tls_session->curr_outmsg_len  = tls_session->outbuf.len - TLS_RECORD_LAYER_HEADER_NBYTES - TLS_HANDSHAKE_HEADER_NBYTES;
    ...
    buf += tls_session->curr_outmsg_len - (TLS_RECORD_LAYER_HEADER_NBYTES + 1 + tls_session->sec.chosen_ciphersuite->tagSize);
    TEST_ASSERT_EQUAL_UINT(buf, (mock_last_hash_in_data[sha384_idx] + mock_last_hash_in_len[sha384_idx]));

    // the second hasdshake message, 4-byte handshake field & 3-byte data field are encoded, the auth tag is not encoded due to
    // insufficient space in outbuf, but the handshake field and data field are not hashed, not encrypted, also not sent.
    ...
    status = tlsTranscrptHashHSmsgUpdate(tls_session, &tls_session->outbuf); // not update transcript hash state
    TEST_ASSERT_EQUAL_INT(TLS_RESP_OK, status);
    TEST_ASSERT_EQUAL_UINT(NULL, mock_last_hash_in_data[sha384_idx]);
    TEST_ASSERT_EQUAL_UINT32(0,  mock_last_hash_in_len[sha384_idx]);

    // the second hasdshake message, previously encoded handshake field and data field are hashed in this fragment.
    tls_session->remain_frags_out = 1;
    tls_session->num_frags_out    = 2;
    tls_session->curr_outmsg_start = 0;
    tls_session->outlen_encoded = TLS_HANDSHAKE_HEADER_NBYTES + 3 + 1 + sec->chosen_ciphersuite->tagSize;
    tls_session->log.last_encode_result = TLS_RESP_OK;
    ...
    // skip the third message, now assume that it's the fourth hasdshake message. Partial data bytes encoded are hashed
    // in current fragment, while rest of data bytes encoded will be hashed in next fragment.
    tls_session->remain_frags_out = 0;
    tls_session->num_frags_out    = 0;
    tls_session->outlen_encoded   = tls_session->outbuf.len - TLS_RECORD_LAYER_HEADER_NBYTES - TLS_HANDSHAKE_HEADER_NBYTES;
    tls_session->outlen_encoded  -= (sec->chosen_ciphersuite->tagSize << 1);
    tls_session->curr_outmsg_start = tls_session->outlen_encoded;
    tls_session->curr_outmsg_len  = TLS_RECORD_LAYER_HEADER_NBYTES + TLS_HANDSHAKE_HEADER_NBYTES;
    tls_session->curr_outmsg_len += (sec->chosen_ciphersuite->tagSize << 1) + 1 + sec->chosen_ciphersuite->tagSize;
    ...
    tls_session->num_frags_out    = 2;
    tls_session->curr_outmsg_start = 0;
    tls_session->outlen_encoded = TLS_HANDSHAKE_HEADER_NBYTES + 1 + sec->chosen_ciphersuite->tagSize;
    tls_session->log.last_encode_result = TLS_RESP_OK;
...
TEST(tlsTranscrptHashHSmsgUpdate, inbuf_multi_frags_one_msg)
{
    ...
    TEST_ASSERT_EQUAL_UINT(buf, (mock_last_hash_in_data[sha256_idx] + mock_last_hash_in_len[sha256_idx]));
...
TEST(tlsTranscrptHashHSmsgUpdate, hs_after_server_finished)
{
    ...
    TEST_ASSERT_EQUAL_STRING_LEN(mock_hash_curr_outbytes[sha256_idx], sec->hashed_hs_msg.snapshot_server_finished, 22);
