...
TEST_GROUP_RUNNER(tlsDeriveTraffickey)
{
    RUN_TEST_CASE(tlsDeriveTraffickey, chk_ok);
}
...
TEST_SETUP(tlsDerivePSKbinderKey)
{
    mock_hkdf_expandlabel_idx = 0;
}
...
TEST_TEAR_DOWN(tlsGenEarlySecret)
{
    mock_hkdf_extract_copydata = NULL;
}
...
TEST(tlsGenEarlySecret, without_psk)
{
    ...
    actual_earlysecret.len  = mqttHashGetOutlenBytes(TLScipherSuiteGetHashID(sec->chosen_ciphersuite));
    ...
    TEST_ASSERT_EQUAL_STRING_LEN(expect_earlysecret, actual_earlysecret.data, actual_earlysecret.len);
...
TEST(tlsDeriveTrafficSecret, tlsDeriveHStrafficSecret)
{
    ...
    earlysecret.len  = mqttHashGetOutlenBytes(TLScipherSuiteGetHashID(tls_session->sec.chosen_ciphersuite));
