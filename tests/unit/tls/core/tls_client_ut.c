...
static const
byte  mock_mqtt_auth_ca_priv_key_rawbyte[] = {
...
static const
byte  mock_mqtt_auth_ca_cert_rawbyte[] = {
...
tlsRespStatus tlsRemoveItemFromList(tlsListItem_t **list, tlsListItem_t *removing_item )
{
    ...
    for(idx=*list; idx!=NULL; idx=idx->next) {
        if(removing_item == idx) {
            if(prev != NULL) { prev->next = removing_item->next; }
            else { *list = removing_item->next; }
            break;
        }
        prev = idx;
    } // end of for-loop
...
TEST_GROUP_RUNNER(tlsClientDeInit)
{
    RUN_TEST_CASE(tlsClientDeInit, deinit_ok);
}
