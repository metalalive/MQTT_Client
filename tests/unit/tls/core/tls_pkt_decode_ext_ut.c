...
static void mock_generate_recv_extension_bytes(
    tlsSession_t *session, tlsExtEntry_t *extlist, word16 nbytes_in_frag
) {
...
static void test_assert_equal_extension_list(tlsExtEntry_t *extitem0, tlsExtEntry_t *extitem1) {
    while (extitem0 != NULL && extitem1 != NULL) {
        ...
        TEST_ASSERT_EQUAL_STRING_LEN(
            extitem0->content.data, extitem1->content.data, extitem1->content.len
        );
...
TEST_GROUP_RUNNER(tlsDecodeExtEncryptExt) { RUN_TEST_CASE(tlsDecodeExtEncryptExt, chk_ok); }

TEST_GROUP_RUNNER(tlsDecodeExtCertReq) { RUN_TEST_CASE(tlsDecodeExtCertReq, chk_ok); }

TEST_GROUP_RUNNER(tlsDecodeExtCertificate) { RUN_TEST_CASE(tlsDecodeExtCertificate, chk_ok); }
...
TEST_SETUP(tlsParseExtensions) {
    ...
    tls_session->last_ext_entry_dec_len =
...
TEST_SETUP(tlsDecodeExtCertificate) {
    ...
    tls_session->peer_certs->exts =
        mock_createEmptyExtensionItem(mock_extension_types[0], mock_extension_content_len[0]);
    tls_session->peer_certs->exts->next =
        mock_createEmptyExtensionItem(mock_extension_types[1], mock_extension_content_len[1]);
...
TEST(tlsParseExtensions, split_1st_ext_item_case2) {
    ...
    TEST_ASSERT_EQUAL_UINT16(
        mock_extension_types[TEST_NUM_EXTENSION_ITEMS - 1], tls_session->exts->type
    );
...
TEST(tlsParseExtensions, split_1st_ext_item_case4) {
    ...
    TEST_ASSERT_EQUAL_UINT16(
        mock_extension_content_len[TEST_NUM_EXTENSION_ITEMS - 1], tls_session->exts->content.len
    );
...
TEST(tlsParseExtensions, split_2nd_ext_item_case1) {
    tlsRespStatus status = TLS_RESP_OK;
    word16        nbytes_in_first_frag =
        2 + 4 + mock_extension_content_len[TEST_NUM_EXTENSION_ITEMS - 1] + 1;
    // write raw bytes to inbuf as received extension section of a TLS record message
    tls_session->inlen_decrypted = tls_session->inbuf.len;
    mock_generate_recv_extension_bytes(tls_session, mock_ext_list, nbytes_in_first_frag);
    // assume to read 1st fragment
    ...
    TEST_ASSERT_EQUAL_UINT16(
        mock_extension_types[TEST_NUM_EXTENSION_ITEMS - 1], tls_session->exts->next->type
    );
    TEST_ASSERT_EQUAL_UINT16(1, tls_session->last_ext_entry_dec_len);
    TEST_ASSERT_EQUAL_UINT16(tls_session->inlen_decrypted, tls_session->inlen_decoded);
    ...
    tls_session->inlen_decoded = 0;
    tls_session->inlen_decrypted = tlsGetExtListSize(mock_ext_list) - 4 -
                                   mock_extension_content_len[TEST_NUM_EXTENSION_ITEMS - 1] - 1;
    status = tlsParseExtensions(tls_session, &tls_session->exts);
    TEST_ASSERT_EQUAL_INT(TLS_RESP_OK, status);
    TEST_ASSERT_EQUAL_UINT16(0, tls_session->last_ext_entry_dec_len);
    test_assert_equal_extension_list(mock_ext_list, tls_session->exts);
...
TEST(tlsParseExtensions, split_2nd_ext_item_case2) {
    tlsRespStatus status = TLS_RESP_OK;
    word16        nbytes_in_first_frag =
        2 + 4 + mock_extension_content_len[TEST_NUM_EXTENSION_ITEMS - 1] + 2;
    // write raw bytes to inbuf as received extension section of a TLS record message
    tls_session->inlen_decrypted = tls_session->inbuf.len;
    mock_generate_recv_extension_bytes(tls_session, mock_ext_list, nbytes_in_first_frag);
    // assume to read 1st fragment
    ...
    TEST_ASSERT_EQUAL_INT(TLS_RESP_OK, status);
    TEST_ASSERT_EQUAL_UINT16(
        mock_extension_types[TEST_NUM_EXTENSION_ITEMS - 2], tls_session->exts->type
    );
    TEST_ASSERT_EQUAL_UINT16(
        mock_extension_types[TEST_NUM_EXTENSION_ITEMS - 1], tls_session->exts->next->type
    );
    ...
    tls_session->inlen_decoded = 0;
    tls_session->inlen_decrypted = tlsGetExtListSize(mock_ext_list) - 4 -
                                   mock_extension_content_len[TEST_NUM_EXTENSION_ITEMS - 1] - 2;
    status = tlsParseExtensions(tls_session, &tls_session->exts);
    TEST_ASSERT_EQUAL_INT(TLS_RESP_OK, status);
    TEST_ASSERT_EQUAL_UINT16(0, tls_session->last_ext_entry_dec_len);
    test_assert_equal_extension_list(mock_ext_list, tls_session->exts);
...
TEST(tlsParseExtensions, split_2nd_ext_item_case3) {
    tlsRespStatus status = TLS_RESP_OK;
    word16        nbytes_in_first_frag =
        2 + 4 + mock_extension_content_len[TEST_NUM_EXTENSION_ITEMS - 1] + 4;
    // write raw bytes to inbuf as received extension section of a TLS record message
    tls_session->inlen_decrypted = tls_session->inbuf.len;
    mock_generate_recv_extension_bytes(tls_session, mock_ext_list, nbytes_in_first_frag);
    // assume to read 1st fragment
    ...
    TEST_ASSERT_EQUAL_UINT16(
        mock_extension_content_len[TEST_NUM_EXTENSION_ITEMS - 2], tls_session->exts->content.len
    );
    TEST_ASSERT_EQUAL_UINT16(
        mock_extension_content_len[TEST_NUM_EXTENSION_ITEMS - 1],
        tls_session->exts->next->content.len
    );
    TEST_ASSERT_EQUAL_UINT16(4, tls_session->last_ext_entry_dec_len);
    TEST_ASSERT_EQUAL_UINT16(tls_session->inlen_decrypted, tls_session->inlen_decoded);
    ...
    tls_session->inlen_decoded = 0;
    tls_session->inlen_decrypted = tlsGetExtListSize(mock_ext_list) - 4 -
                                   mock_extension_content_len[TEST_NUM_EXTENSION_ITEMS - 1] - 4;
    status = tlsParseExtensions(tls_session, &tls_session->exts);
    TEST_ASSERT_EQUAL_INT(TLS_RESP_OK, status);
    TEST_ASSERT_EQUAL_UINT16(0, tls_session->last_ext_entry_dec_len);
    test_assert_equal_extension_list(mock_ext_list, tls_session->exts);
...
TEST(tlsParseExtensions, split_2nd_ext_item_encrypt) {
    ...
    tlsRespStatus status = TLS_RESP_OK;
    word16        nbytes_in_first_frag =
        2 + 4 + mock_extension_content_len[TEST_NUM_EXTENSION_ITEMS - 1] + 5;
    // write raw bytes to inbuf as received extension section of a TLS record message
    tls_session->inlen_decrypted = tls_session->inbuf.len;
    mock_generate_recv_extension_bytes(tls_session, mock_ext_list, nbytes_in_first_frag);
    // assume to read 1st fragment
    ...
    tls_session->inlen_decoded = 0;
    tls_session->inlen_decrypted = tlsGetExtListSize(mock_ext_list) - 4 -
                                   mock_extension_content_len[TEST_NUM_EXTENSION_ITEMS - 1] - 5;
