...
{
    byte  record_header[TLS_RECORD_LAYER_HEADER_NBYTES] = { TLS_CONTENT_TYPE_APP_DATA, 0x3, 0x3, 0x0, 0x0, };
    ...
    for(nbytes_msg_body = 2; nbytes_msg_body < (tls_session->sec.chosen_ciphersuite->tagSize << 1); nbytes_msg_body++)
    { // start of loop nbytes_msg_body
        ...
        expect_num_msgs = 0;    actual_num_msgs = 0;
        XMEMSET(&mock_sys_pkt_write_content[0], 0x00, MAX_RAWBYTE_BUF_SZ);
        ...
        TEST_ASSERT_EQUAL_INT(TLS_RESP_OK, status);
        if(tls_session->num_frags_out > 0) {  TEST_ASSERT_EQUAL_UINT8(2, tls_session->num_frags_out); }
        TEST_ASSERT_EQUAL_UINT16(nbytes_encoded_not_encrypted, tls_session->outlen_encoded);
        ...
        for(idx = 0; idx < MAX_RAWBYTE_BUF_SZ; idx += nbytes_per_record_msg) {
            if(XSTRNCMP((const char *)&record_header[0], (const char *)buf, TLS_RECORD_LAYER_HEADER_NBYTES) == 0) {
...
TEST(tlsPktRecvFromPeer, err_chk)
{
    ...
    nbytes_per_record_msg  = TLS_RECORD_LAYER_HEADER_NBYTES + TLS_MAX_BYTES_RECORD_LAYER_PKT + 1; // message size that exceeds limit
...
{
    byte  record_header[TLS_RECORD_LAYER_HEADER_NBYTES] = { TLS_CONTENT_TYPE_APP_DATA, 0x3, 0x3, 0x0, 0x0, };
    byte  expect_lastbyte_msgs[5] = {0, };
    byte  actual_lastbyte_msgs[5] = {0, };
    word16  nbytes_per_record_msg = 0;
    word16  idx = 0, jdx = 1;
    tlsRespStatus status = TLS_RESP_OK;
...
TEST(tlsPktRecvFromPeer, one_msg_multi_fragments)
{
    ...
    nbytes_per_record_msg = tls_session->inbuf.len * 3 - (tls_session->sec.chosen_ciphersuite->tagSize) - 1;
    ...
    *buf++ = TLS_CONTENT_TYPE_APP_DATA;
    ...
    do { // -------------- receive / check all the fragments of the record message --------------
        if ((tls_session->inlen_total > 0) && (tls_session->inlen_total < tls_session->sec.chosen_ciphersuite->tagSize)) {
            // part of auth tag is already copied in current fragment, but there's still part of it in
            // the final fragment which will be loaded later
            nbytes_recv_not_decrypted = tls_session->sec.chosen_ciphersuite->tagSize - tls_session->inlen_total;
        } else {
            nbytes_recv_not_decrypted = (tls_session->inlen_unprocessed - TLS_RECORD_LAYER_HEADER_NBYTES) % tls_session->sec.chosen_ciphersuite->tagSize;
            idx++;
        }
        ...
        nbytes_cpy = XMIN(tls_session->inbuf.len - nbytes_recv_not_decrypted, nbytes_per_record_msg - nbytes_recved);
        ...
        TEST_ASSERT_EQUAL_STRING_LEN(&mock_sys_pkt_read_content[0], &tls_session->inbuf.data[nbytes_recv_not_decrypted], nbytes_cpy);
