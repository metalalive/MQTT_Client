...
int sha256_init(hash_state *md) { return 0; }

int sha384_init(hash_state *md) { return 0; }
...
void rijndael_done(symmetric_key *skey) { return; }

int rijndael_enc_keysize(int *keysize) { return 0; }

int chacha20poly1305_init(
    chacha20poly1305_state *st, const unsigned char *key, unsigned long keylen
) {
    return 0;
}

int gcm_init(gcm_state *gcm, int cipher, const unsigned char *key, int keylen) { return 0; }

int gcm_reset(gcm_state *gcm) { return 0; }

int gcm_add_iv(gcm_state *gcm, const unsigned char *IV, unsigned long IVlen) { return 0; }

int gcm_add_aad(gcm_state *gcm, const unsigned char *adata, unsigned long adatalen) { return 0; }
...
int ecc_make_key(prng_state *prng, int wprng, int keysize, ecc_key *key) { return 0; }

void ecc_free(ecc_key *key) { return; }
...
int ecc_ansi_x963_import_ex(
    const unsigned char *in, unsigned long inlen, ecc_key *key, const ltc_ecc_curve *cu
) {
    if (key != NULL && in != NULL) {
...
int x25519_make_key(prng_state *prng, int wprng, curve25519_key *key) { return 0; }
...
int x25519_import_raw(
    const unsigned char *in, unsigned long inlen, int which, curve25519_key *key
) {
    if (key != NULL && in != NULL) {
...
int ecc_shared_secret(
    const ecc_key *private_key, const ecc_key *public_key, unsigned char *out, unsigned long *outlen
) {
    return 0;
}

int x25519_shared_secret(
    const curve25519_key *private_key, const curve25519_key *public_key, unsigned char *out,
...
int hmac_memory(
    int hash, const unsigned char *key, unsigned long keylen, const unsigned char *in,
...
int rsa_verify_hash_ex(
    const unsigned char *sig, unsigned long siglen, const unsigned char *hash,
    unsigned long hashlen, int padding, int hash_idx, unsigned long saltlen, int *stat,
    const rsa_key *key
) {
    if (stat != NULL) {
        *stat = 1;
...
int rsa_sign_hash_ex(
    ...
    int padding, prng_state *prng, int prng_idx, int hash_idx, unsigned long saltlen,
    const rsa_key *key
) {
    return 0;
}
