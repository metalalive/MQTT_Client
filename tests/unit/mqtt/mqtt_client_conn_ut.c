...
static byte mock_rawbytes_auth[0xe] = {
    0xf0,
    0x0c,
    0x18,
    0x0a, // packet ID & length field of property
    MQTT_PROP_AUTH_METHOD,
    0x00,
    0x02,
    0x5e,
    0xe4, // fake auth method property
    MQTT_PROP_AUTH_DATA,
    0x00,
    0x02,
    0xbe,
    0xef
}; // fake auth data property
static byte mock_rawbytes_invalid_cmd[0x4] = {0x00, 0x02, 0xde, 0xad};
static byte mock_rawbytes_pub_qos0[0xc] = {
    0x30, 0x0a, 0x00, 0x03, 0x62, 0x63, 0x64, // topic : bcd
    0x00,                                     // no property included
    0x42, 0x31, 0x38, 0x32                    // message : B182
};

// ---------------- mock or dummy functions declaration ------------------
mqttRespStatus mqttSysInit(void) { return mock_sysinit_return_val; }
    ...
    case MQTT_PACKET_TYPE_PUBCOMP:
        ...
        *recv_pkt_id = mock_mqtt_pkt_id;
        if ((cmdtype == MQTT_PACKET_TYPE_PUBRECV) || (cmdtype == MQTT_PACKET_TYPE_PUBREL)) {
            ...
            mock_decode_pkt_return_val =
                mqttSendPubResp(mctx, (cmdtype + 1), (mqttPktPubResp_t **)p_decode);
        }
        break;
...
int mqttGetPktLenConnect(mqttConn_t *conn, word32 max_pkt_sz) { return mock_get_pktlen_return_val; }

int mqttGetPktLenAuth(mqttAuth_t *auth, word32 max_pkt_sz) { return mock_get_pktlen_return_val; }

int mqttGetPktLenDisconn(mqttPktDisconn_t *disconn, word32 max_pkt_sz) {
    return mock_get_pktlen_return_val;
}

int mqttGetPktLenPublish(mqttMsg_t *msg, word32 max_pkt_sz) { return mock_get_pktlen_return_val; }
...
mock_mqttAuthFinalCallback(mqttStr_t *auth_data_sent, mqttStr_t *auth_reason_out) {
    return MQTT_RESP_OK;
...
TEST_GROUP_RUNNER(mqttPropertyDel) { RUN_TEST_CASE(mqttPropertyDel, take_some_alloc_space); }
...
TEST_GROUP_RUNNER(mqttSendDisconnect) { RUN_TEST_CASE(mqttSendDisconnect, err_chk); }
...
TEST_GROUP_RUNNER(mqttSendUnsubscribe) { RUN_TEST_CASE(mqttSendUnsubscribe, topics_sent); }

TEST_GROUP_RUNNER(mqttSendPingReq) { RUN_TEST_CASE(mqttSendPingReq, ping_sent); }
...
TEST(mqttPropertyDel, take_some_alloc_space) {
    ...
    for (idx = 0; idx < MQTT_MAX_NUM_PROPS; idx++) {
        switch (idx % 3) {
        case 1:
            ptype = MQTT_PROP_MSG_EXPIRY_INTVL;
            break;
        case 2:
...
TEST(mqttPropErrChk, chk_topic_alias_pubmsg) {
    ...
    { // assume the client gets this property in the PUBLISH command that is ready to send out
        ...
        TEST_ASSERT_EQUAL_UINT8(
            MQTT_REASON_TOPIC_ALIAS_INVALID, unittest_mctx->err_info.reason_code
        );
...
TEST(mqttPropErrChk, chk_subs_id) {
    ...
    TEST_ASSERT_EQUAL_INT(
        MQTT_RESP_ERR_PROP, status
    ); // root cause: sending PUBLISH with SUBSCRIBE_ID property
    TEST_ASSERT_EQUAL_UINT8(MQTT_REASON_PROTOCOL_ERR, unittest_mctx->err_info.reason_code);
