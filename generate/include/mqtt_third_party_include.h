...
{ \
    ...
    int __status = hmac_memory(hash_idx, \
...
{ \
    ...
    int __status = ecc_make_key((prng_state *)(drbg), prng_list_idx, (keysize), (tlsECCkey_t *)(key)); \
...
{ \
    ...
    int  __status  = ecc_ansi_x963_export((const tlsECCkey_t *)(key), (outbuf), (unsigned long *)&export_sz); \
...
{ \
    int __status = ecc_ansi_x963_import_ex((const byte *)(inbuf), (word32)(inbuflen), (tlsECCkey_t *)(key), (cu)); \
    (tlsstatus) = (__status == CRYPT_OK ? TLS_RESP_OK: TLS_RESP_ERR_KEYGEN); \
}
...
{ \
    ...
    switch((grpid)) { \
        case TLS_NAMED_GRP_SECP256R1: \
            curve_name = "SECP256R1"; break; \
        case TLS_NAMED_GRP_SECP384R1: \
            curve_name = "SECP384R1"; break; \
        case TLS_NAMED_GRP_SECP521R1: \
            curve_name = "SECP521R1"; break; \
        default: __status = CRYPT_INVALID_ARG; break; \
    } \
...
{ \
    ...
    int __status = ecc_shared_secret((const tlsECCkey_t *)(local_key), (const tlsECCkey_t *)(remote_key), \
...
{ \
    const int  prng_list_idx = 0; \
    int __status = x25519_make_key((prng_state *)(drbg), prng_list_idx, (tlsX25519Key_t *)(key)); \
    (tlsstatus) = (__status == CRYPT_OK ? TLS_RESP_OK: TLS_RESP_ERR_KEYGEN); \
}
...
{ \
    unsigned long outlen_tmp = (outbuflen); \
    int __status = x25519_shared_secret((const tlsX25519Key_t *)(local_key), \
                                        (const tlsX25519Key_t *)(remote_key), \
                                        (byte *)(outbuf), &outlen_tmp ); \
    XASSERT(outlen_tmp == (unsigned long)(outbuflen)); \
    (tlsstatus) = (__status == CRYPT_OK ? TLS_RESP_OK: TLS_RESP_ERR_KEYGEN); \
}
...
#define  TLS_CFG_RSA_IMPORT_PRIVKEY_FN( tlsstatus, privkey, start_N, sz_N, start_e,  sz_e,  start_d,  sz_d,  start_p,  sz_p,\
                                        start_q,  sz_q,  start_dP,  sz_dP, start_dQ, sz_dQ, start_qP, sz_qP ) \
{ \
    tlsRSAkey_t *key = (tlsRSAkey_t *)(privkey); \
    mp_err  __status = MP_OKAY; \
    __status |= mp_from_ubin((mp_int *)key->e  , (start_e ), (sz_e )); \
...
#define  TLS_CFG_RSA_SIGN_SIGNATURE_FN(tlsstatus, drbg, indata, inlen, signalgo, outdata, outlen, key, rsapss_hash_id, rsapss_saltlen) \
{ \
    unsigned long outlen_tmp = (outlen); \
    ...
    int __status = rsa_sign_hash_ex((const byte *)(indata), (inlen), (byte *)(outdata), &outlen_tmp, padding_mthd, \
...
#define  TLS_CFG_RSA_VERIFY_SIGN_FN(tlsstatus, signdata, signlen, signalgo, hashdata, hashlen, key, rsapss_hash_id, rsapss_saltlen) \
{ \
    int match_status = 0; \
    int hash_idx = TAB_SIZE; \
    int padding_mthd = 0; \
    ...
    int __status = rsa_verify_hash_ex((const byte *)(signdata), (signlen),  (const byte *)(hashdata), (hashlen), \
                                      padding_mthd, hash_idx, (rsapss_saltlen),  &match_status, \
                                      (const tlsRSAkey_t *)(key)); \
    (tlsstatus) = ((__status == CRYPT_OK) && (match_status == 1) ? TLS_RESP_OK: TLS_RESP_CERT_AUTH_FAIL); \
}
...
{ \
    ...
    int __status = gcm_init((gcm_state *)(ctx), cipher_idx, (const byte *)(key_str), (int)(keysize)); \
...
{ \
    int __status = gcm_add_iv((gcm_state *)(ctx), (const byte *)(iv_str), (unsigned long)(iv_size)); \
    (tlsstatus) = (__status == CRYPT_OK ? TLS_RESP_OK: TLS_RESP_ERR_ENCRYPT); \
}

#define  TLS_CFG_AES_GCM_SET_AAD_FN(tlsstatus, ctx, aad_str, aad_size) \
{ \
    int __status = gcm_add_aad((gcm_state *)(ctx), (const byte *)(aad_str), (unsigned long)(aad_size)); \
    (tlsstatus) = (__status == CRYPT_OK ? TLS_RESP_OK: TLS_RESP_ERR_ENCRYPT); \
}
...
{ \
    ...
    int __status = gcm_process((gcm_state *)(ctx), (pt), (unsigned long)(len), (ct), direction); \
    (tlsstatus) = (__status == CRYPT_OK ? TLS_RESP_OK: TLS_RESP_ERR_ENCRYPT); \
}
...
{ \
    int __status = chacha20poly1305_init((chacha20poly1305_state *)(key_ctx), (const byte *)(key_str), (word32)(keysize)); \
    (tlsstatus) = (__status == CRYPT_OK ? TLS_RESP_OK: TLS_RESP_ERR_KEYGEN); \
}
