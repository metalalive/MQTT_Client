...
tlsRespStatus  tlsRSAgetPrivKey(const byte *in, word16 inlen, void **privkey_p)
{
    ...
    if(obj_data_sz != 1 || *in++ > 0x1) {
        status = TLS_RESP_ERR_PARSE; goto done;
    }
    ...
    if(status < 0) { goto done; }
    TLS_CFG_RSA_IMPORT_PRIVKEY_FN( status, *privkey_p, element_offset[0], element_sz[0], element_offset[1], element_sz[1],
                                   element_offset[2], element_sz[2], element_offset[3], element_sz[3],
                                   element_offset[4], element_sz[4], element_offset[5], element_sz[5],
                                   element_offset[6], element_sz[6], element_offset[7], element_sz[7] );
done:
    return status;
