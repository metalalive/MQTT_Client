...
static  byte  tlsGetInnertextRecType(tlsSession_t *session, const byte *innertext, word32 len)
{
    ...
    byte  out  = 0;
    // TODO: figure out better way to handle post-handshake messages like KeyUpdate or NewSessionTicket
    // after handshake process is done, They may be sent from peer between application data messages.
    // Currently this implementation can ONLY recognize post-handshake messages which fit into
    // one single fragment.
    ...
    } // For record messages consisting of multiple network fragments, check the flags & guess the record type
    else {
...
tlsRespStatus  tlsEncryptRecordMsg(tlsSession_t *session)
{
    ...
    if(session->log.last_encode_result == TLS_RESP_OK) { // implicit meaning : it's the final fragment
    ...
    status = session->sec.chosen_ciphersuite->encrypt_fn( &session->sec, ct_start, ct_start, &ct_len );
...
tlsRespStatus  tlsDecryptRecordMsg(tlsSession_t *session)
{
    ...
    if(status == TLS_RESP_REQ_REINIT) { return TLS_RESP_ERR; }
    ...
    // if we have next fragment smaller than authentication tag size, that means current fragment contains part
    // of authentication tag, then we can reduce the length of current ciphertext to decrypt in advance, 
    // then next (final) fragment will verify the tag & decrypt final ciphertext block.
    if ((session->inlen_total > 0) && (session->inlen_total < session->sec.chosen_ciphersuite->tagSize)) {
        ct_len -= (session->sec.chosen_ciphersuite->tagSize - session->inlen_total);
    }
    status = session->sec.chosen_ciphersuite->decrypt_fn( &session->sec, ct_start, ct_start, &ct_len );
    if(status >= 0) { // post processing after decryption succeed
        if(session->sec.flgs.ct_first_frag != 0) {
            // update actual record type TLSInnerPlaintext.type
            session->inbuf.data[0] = tlsGetInnertextRecType(session, (const byte *)ct_start, ct_len);
        }
        if(session->sec.flgs.ct_final_frag != 0) {
