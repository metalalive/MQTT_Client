...
tlsX509ChkVersion(byte *in, word32 *inlen, tlsX509versionCode expected_ver, word32 *datalen) {
    ...
    status = tlsASN1GetIDlen(
        in, &obj_idlen_sz, (ASN_TAG_CONSTRUCTED | ASN_TAG_CONTEXT_SPECIFIC), &obj_data_sz
    );
    if (status < 0) {
        goto done;
    }
...
static tlsRespStatus
tlsX509GetDNattributes(byte *in, word32 *inlen, tlsCertProfile_t *prof, word32 *datalen) {
    ...
    *datalen = obj_data_sz;
    ...
    while ((remain_sz > 0) && (status == TLS_RESP_OK)) {
        ...
        status = tlsASN1GetIDlen(
            in, &obj_idlen_sz, (ASN_PRIMDATA_SET | ASN_TAG_CONSTRUCTED), &obj_data_sz
        );
        if (status < 0) {
            break;
        }
        ...
        if (XSTRNCMP((const char *)&in[0], (const char *)&oid_dn_id_at[0], 4) == 0) {
            ...
            if (dst != NULL) {
                ...
                if (status == TLS_RESP_OK) {
                    ...
                    in += obj_idlen_sz;
                    if (*dst == NULL) {
                        *dst = XMALLOC(sizeof(byte) * obj_data_sz);
                    }
...
static tlsRespStatus tlsX509ChkValidTime(byte *in, word32 *inlen, word32 *datalen) {
    ...
    word32        obj_data_sz = 0;
    tlsRespStatus status = TLS_RESP_OK;
    // time format : convert to Binary Coded Decimal, 4-bit digits as YYYYMMDDHHMM (Generalized
    // Time), or YYMMDDHHMM (UTC time) skip "seconds" part in this implementation.
    byte notbefore[6]; // starting date after a certificate is valid
    byte notafter[6];  // expiration data before a certificate is valid, format: byte 2: YYYY, byte
                       // 1: MM, byte 0: DD
    byte bias = 0;
    ...
    *datalen = obj_data_sz;
    in += obj_idlen_sz;
    obj_idlen_sz = obj_data_sz;
    // RFC 5280 suggests the certificates extended across 2050 year will use generalized time
    // instead of UTC time.
    switch (in[0]) {
    ...
    byte tmp = 0;
    in += obj_idlen_sz;
    // note that last 3 bytes of both generalized time and UTC time are "second digits" and "z"
    // letter, which are NOT used in this implementation.
...
static tlsRespStatus tlsX509getPublicKey(
    byte *in, word32 *inlen, tlsAlgoOID *pubkey_algo, void **pubkey_p, word32 *datalen
) {
    word32        obj_idlen_sz = 0;
    word32        obj_data_sz = 0;
    ...
    *datalen = obj_data_sz;
    ...
    obj_idlen_sz = remain_sz;
    // Note currently this implementation ONLY supports RSA public key algorithm in certificate
    // verification
    switch (*pubkey_algo) {
...
static tlsRespStatus
tlsX509getSignature(byte *in, word32 *inlen, tlsOpaque16b_t *out, word32 *datalen) {
    word32        obj_idlen_sz = 0;
    word32        obj_data_sz = 0;
    word32        remain_sz = *inlen;
    tlsRespStatus status = TLS_RESP_OK;
...
static tlsRespStatus
tlsX509HashCertInfo(tlsOpaque16b_t *out, tlsHashAlgoID hash_id, byte *in, word32 inlen) {
...
static tlsRespStatus
tlsX509GetRSAPSSextraInfo(byte *in, word32 *inlen, tlsRSApss_t *out, word32 *datalen) {
    word32        obj_idlen_sz = 0;
    word32        obj_data_sz = 0;
    word32        remain_sz = *inlen;
    tlsRespStatus status = TLS_RESP_OK;
        ...
        if ((in[0] & 0xfc) == (ASN_TAG_CONTEXT_SPECIFIC | ASN_TAG_CONSTRUCTED)) {
            ...
            if (tmp == idx) {
                ...
                case 2: // get salt length
                    obj_idlen_sz = remain_sz;
                    status =
                        tlsASN1GetIDlen(in, &obj_idlen_sz, (ASN_PRIMDATA_INTEGER), &obj_data_sz);
                    if (status < 0 || obj_data_sz != 2) {
                        goto done;
                    }
                    remain_sz -= obj_idlen_sz;
                    in += obj_idlen_sz;
...
tlsRespStatus tlsDecodeX509cert(tlsCert_t *cert) {
    ...
    status = tlsASN1GetIDlen(
        &cert->rawbytes.data[decoded], &obj_idlen_sz, (ASN_PRIMDATA_SEQUENCE | ASN_TAG_CONSTRUCTED),
        &obj_data_sz
    );
    if (status < 0) {
        goto done;
    ...
    // * store starting address & size of the first part of the certificate, for later hash
    // operation
    obj_idlen_sz = cert_len - decoded;
    status = tlsASN1GetIDlen(
        &cert->rawbytes.data[decoded], &obj_idlen_sz, (ASN_PRIMDATA_SEQUENCE | ASN_TAG_CONSTRUCTED),
    ...
    // * check certificate version, this implementation ONLY support x509v3 and don't accept
    // negotiation on this.
    obj_idlen_sz = cert_len - decoded;
    ...
    obj_idlen_sz = cert_len - decoded;
    status = tlsASN1GetIDlen(
        &cert->rawbytes.data[decoded], &obj_idlen_sz, (ASN_PRIMDATA_INTEGER), &obj_data_sz
    );
    if (status < 0) {
        goto done;
    }
    ...
    obj_idlen_sz = cert_len - decoded;
    status = tlsASN1GetAlgoID(
        &cert->rawbytes.data[decoded], &obj_idlen_sz, &cert->cert_algo, &obj_data_sz
    );
    if (status < 0) {
        goto done;
    }
    ...
    if (cert->cert_algo == TLS_ALGO_OID_RSASSA_PSS) {
        obj_idlen_sz = cert_len - decoded;
        status = tlsX509GetRSAPSSextraInfo(
            &cert->rawbytes.data[decoded], &obj_idlen_sz, &cert->rsapss, &obj_data_sz
        );
        if (status < 0) {
            goto done;
        }
    ...
    // hash distinguished name (DN) attributes for issuer (e.g. CA, any intermediate trustee) or
    // subject (e.g. the perr, server) store few useful information, then hash entire DN section for
    // later verification.
    obj_idlen_sz = cert_len - decoded;
    status = tlsX509GetDNattributes(
        &cert->rawbytes.data[decoded], &obj_idlen_sz, &cert->issuer, &obj_data_sz
    );
    if (status < 0) {
        goto done;
    }
    ...
    // TODO: find better way to get current generalized date time, client could be on
    ...
    obj_idlen_sz = cert_len - decoded;
    status = tlsX509GetDNattributes(
        &cert->rawbytes.data[decoded], &obj_idlen_sz, &cert->subject, &obj_data_sz
    );
    if (status < 0) {
        goto done;
    }
    ...
    // get public key
    obj_idlen_sz = cert_len - decoded;
    status = tlsX509getPublicKey(
        &cert->rawbytes.data[decoded], &obj_idlen_sz, &cert->pubkey_algo, &cert->pubkey,
        &obj_data_sz
    );
    if (status < 0) {
        goto done;
    ...
    obj_idlen_sz = cert_len - decoded;
    status = tlsX509getExtensions(
        &cert->rawbytes.data[decoded], &obj_idlen_sz, (tlsX509v3ext_t **)&cert->cert_exts,
        &obj_data_sz
    );
    if (status < 0) {
        goto done;
    ...
    obj_idlen_sz = cert_len - decoded;
    status = tlsASN1GetAlgoID(
        &cert->rawbytes.data[decoded], &obj_idlen_sz, &cert->sign_algo, &obj_data_sz
    );
    if (status < 0) {
        goto done;
    }
    ...
    if (cert->sign_algo == TLS_ALGO_OID_RSASSA_PSS) {
        ...
        obj_idlen_sz = cert_len - decoded;
        status = tlsX509GetRSAPSSextraInfo(
            &cert->rawbytes.data[decoded], &obj_idlen_sz, &rsapss_sign, &obj_data_sz
        );
        if (status < 0) {
            goto done;
        }
        if (XSTRNCMP(
                (const char *)&cert->rsapss, (const char *)&rsapss_sign, sizeof(tlsRSApss_t)
            ) != 0) {
            status = TLS_RESP_ERR_DECODE;
            goto done;
        }
    ...
    obj_idlen_sz = cert_len - decoded;
    status = tlsX509getSignature(
        &cert->rawbytes.data[decoded], &obj_idlen_sz, &cert->signature, &obj_data_sz
    );
    if (status < 0) {
        goto done;
    }
    ...
    // hash certificate holder information (first part of the cert, excluding signature) for later
    // verification
    tlsHashAlgoID hash_id = (cert->cert_algo == TLS_ALGO_OID_RSASSA_PSS)
                                ? cert->rsapss.hash_id
                                : tlsX509getHashIDfromAlgoID(cert->cert_algo);
    status = tlsX509HashCertInfo(
        &cert->hashed_holder_info, hash_id, cert_hodler_info_start, cert_hodler_info_len
    );
