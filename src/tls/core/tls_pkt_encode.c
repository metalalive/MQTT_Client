...
tlsGenFinishedVerifyData(tlsSecurityElements_t *sec, tlsOpaque8b_t *base_key, tlsOpaque8b_t *out) {
    ...
    if (sec == NULL || sec->chosen_ciphersuite == NULL || base_key == NULL ||
        base_key->data == NULL || out == NULL || out->data == NULL) {
    ...
    TLS_CFG_HMAC_MEMBLOCK_FN(
        status, hash_id, finishedkey.data, hash_len, trHash_hs_msg, hash_len, &out->data[0],
        out->len
    );
done:
...
static tlsRespStatus tlsEncodeHSclientHello(tlsSession_t *session) {
    ...
    if (tlsChkFragStateOutMsg(session) == TLS_RESP_REQ_REINIT) {
        ...
        mqttstatus = mqttUtilRandByteSeq(
            session->drbg, &session->sec.client_rand[0], (word16)TLS_HS_RANDOM_BYTES
        );
        if (mqttstatus != MQTT_RESP_OK) {
            goto encode_failure;
        }
        ...
        mqttstatus = mqttUtilRandByteSeq(
            session->drbg, &session->tmpbuf.session_id.data[0], TLS_MAX_BYTES_SESSION_ID
        );
        if (mqttstatus != MQTT_RESP_OK) {
            goto encode_failure;
        }
        ...
        XMEMCPY(
            &outbuf[outlen_encoded], &session->tmpbuf.session_id.data[0], TLS_MAX_BYTES_SESSION_ID
        );
        outlen_encoded += TLS_MAX_BYTES_SESSION_ID;
        // append a list of supported cipher suite. In practice , a ClientHello TLS packet can
        // contain the entire list of cipher suites, it is unlikely to divide the entire list & send
        // it in multiple TLS packets.
        ...
        session->curr_outmsg_len =
            outlen_encoded - session->curr_outmsg_start + 2 + session->ext_enc_total_len;
...
static tlsRespStatus tlsEncodeHScertificate(tlsSession_t *session) {
    ...
    if ((session->nbytes.remaining_to_send == 0) &&
        ((session->last_ext_entry_enc_len >> 15) != 0x0)) {
...
static tlsRespStatus tlsEncodeHScertificateVerify(tlsSession_t *session) {
    ...
    if (tlsChkFragStateOutMsg(session) == TLS_RESP_REQ_REINIT) {
        ...
        status = tlsCertVerifyGenDigitalSig(
            &session->sec, (const tlsRSApss_t *)&rsapssSig, &digiSig, (const byte)0x0
        );
        if (status < 0) {
            goto done;
        }
        ...
        outlen_encoded +=
            tlsEncodeWord16(&outbuf[outlen_encoded], TLS_SIGNATURE_RSA_PSS_RSAE_SHA256);
        outlen_encoded += tlsEncodeWord16(&outbuf[outlen_encoded], session->client_signed_sig.len);
        session->curr_outmsg_len =
            outlen_encoded - session->curr_outmsg_start + session->client_signed_sig.len;
...
static tlsRespStatus tlsEncodeHSfinished(tlsSession_t *session) {
    ...
    if (tlsChkFragStateOutMsg(session) == TLS_RESP_REQ_REINIT) {
        ...
        session->tmpbuf.finish_verifydata.data =
            XMALLOC(sizeof(byte) * session->tmpbuf.finish_verifydata.len);
        base_key = &session->sec.secret.hs.client;
        status =
            tlsGenFinishedVerifyData(&session->sec, base_key, &session->tmpbuf.finish_verifydata);
        if (status < 0) {
            goto done;
        }
        session->nbytes.remaining_to_send = session->tmpbuf.finish_verifydata.len;
        session->curr_outmsg_len =
            outlen_encoded - session->curr_outmsg_start + session->tmpbuf.finish_verifydata.len;
    }
    if (session->nbytes.remaining_to_send > 0) {
        copied_sz = session->tmpbuf.finish_verifydata.len - session->nbytes.remaining_to_send;
        rdy_cpy_sz = XMIN(session->outbuf.len - outlen_encoded, session->nbytes.remaining_to_send);
        XMEMCPY(
            &outbuf[outlen_encoded], &session->tmpbuf.finish_verifydata.data[copied_sz], rdy_cpy_sz
        );
        outlen_encoded += rdy_cpy_sz;
        session->nbytes.remaining_to_send -= rdy_cpy_sz;
        if (outlen_encoded == session->outbuf.len) {
...
static tlsRespStatus tlsEncodeAppData(tlsSession_t *session) {
    ...
    if (tlsChkFragStateOutMsg(session) == TLS_RESP_REQ_REINIT) {
        session->curr_outmsg_len =
            outlen_encoded - session->curr_outmsg_start + session->app_pt.len;
...
static void tlsEncodeRecordHeader(tlsSession_t *session
    ...
    tlsRecordLayer_t *rec_header =
        (tlsRecordLayer_t *)&session->outbuf.data[session->curr_outmsg_start];
    ...
    if (session->flgs.hs_tx_encrypt == 0) { // for TLSplainText
        ...
        // add size for final part of TLSinnerPlainText (1-byte content-type, and variable-bytes
        // authentication tag)
    ...
    tlsEncodeWord16(
        (byte *)&rec_header->fragment.len,
        (session->curr_outmsg_len - TLS_RECORD_LAYER_HEADER_NBYTES)
    );
} // end of tlsEncodeRecordHeader

static tlsRespStatus tlsEncodeInnerPlainTextFooter(tlsSession_t *session
) { // this function may add up number of fragments if the required extra bytes of the footer cannot
    // be fit
    // in current out-flight message (outbuf), in such case this function must return
    // TLS_RESP_REQ_MOREDATA
    tlsRespStatus status = TLS_RESP_OK;
    word16        outlen_encoded = session->outlen_encoded;
    int           avail_buf_sz =
        session->outbuf.len - outlen_encoded - (1 + session->sec.chosen_ciphersuite->tagSize);
    if (avail_buf_sz >= 0) {
        session->outbuf.data[outlen_encoded++] = session->record_type;
        // still preserve last few bytes for authentication tag, which will be appended to outbuf
        // later in encryotion function
...
tlsRespStatus tlsEncodeRecordLayer(tlsSession_t *session) {
    ...
    if (frag_status == TLS_RESP_REQ_REINIT) { // runs only for the first fragment
        if (session->outlen_encoded > (session->outbuf.len - TLS_RECORD_LAYER_HEADER_NBYTES)) {
            return TLS_RESP_ERRMEM;
        } // report memory error because no sufficient space in outbuf for placing 5-byte record
          // header
        session->curr_outmsg_start = session->outlen_encoded;
        session->outlen_encoded += TLS_RECORD_LAYER_HEADER_NBYTES;
    } else { // if this encoding message split to several fragments (and the first fragment was
             // already sent)
