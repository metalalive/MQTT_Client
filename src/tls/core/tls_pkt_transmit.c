...
tlsRespStatus tlsPktSendToPeer(tlsSession_t *session, byte flush_flg)
{
    ...
    if(session->flgs.hs_tx_encrypt == 0) { session->outlen_encrypted = session->outlen_encoded; }
    rdy_nbytes = session->outlen_encrypted;
    if(rdy_nbytes > session->outbuf.len ) {
        status = TLS_RESP_ERRMEM; goto end_of_pkt_write;
    }
    ...
    // if flush flag is NOT set, and there are less than 26 bytes avaiable, then we simply place encoded
    // or encrypted bytes in outbuf, set them at later time when there's more data bytes written in.
    if((flush_flg == 0) && (buf_len >= wr_len)) {
        ...
        // the current TLS record message may be too large to fit in out-flight buffer (a TCP packet), it is
        // split into multiple fragments, in that case we will encode and send the subsequent fragments
...
tlsRespStatus tlsPktRecvFromPeer(tlsSession_t *session)
{
    ...
    { // the first fragment of in-flight message must contain record header
        const tlsRecordLayer_t *rec_header = (tlsRecordLayer_t *) buf;
        // read first 5 byte as header of the new TLS record message
        rd_len = mqttSysPktRead( &session->ext_sysobjs[0], buf,
                       TLS_RECORD_LAYER_HEADER_NBYTES, session->cmd_timeout_ms );
        if(rd_len < 0) {
        ...
        if(status < 0) { goto end_of_pkt_read; }
        ...
        if(session->inlen_unprocessed > 0) {
            // adjust in-flight buffer if there is still received-but-undecrypted bytes (means we have more bytes
            // to receive in next fragment) (by calling memmove ??)
            XMEMMOVE((void *)buf, (void *)&buf[session->inlen_decrypted], (size_t)session->inlen_unprocessed );
    ...
    do { // load message (in one or several rounds)
        rd_len = mqttSysPktRead( &session->ext_sysobjs[0], &buf[session->inlen_unprocessed],
                                 cpy_len, session->cmd_timeout_ms );
        if(rd_len < 0) {
            status = tlsRespCvtFromMqttResp((mqttRespStatus)rd_len);
            goto end_of_pkt_read;
        }
...
void  tlsDecrementFragNumOutMsg(tlsSession_t *session)
{
    if((session != NULL) && (session->remain_frags_out > 0)) {
        session->remain_frags_out -= 1;
        if(session->remain_frags_out == 0) { session->num_frags_out = 0; }
    }
...
void  tlsDecrementFragNumInMsg(tlsSession_t *session)
{
    if((session != NULL) && (session->remain_frags_in > 0)) {
        session->remain_frags_in -= 1;
        if(session->remain_frags_in == 0) { session->num_frags_in = 0; }
    }
