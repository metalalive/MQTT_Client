...
//
static tlsRespStatus  tlsDeriveSecret(tlsHashAlgoID hash_id, tlsOpaque8b_t *in_secret, tlsOpaque8b_t *label,
                                      tlsOpaque8b_t  *trHashMsg, tlsOpaque8b_t *out_secret)
{
    ...
    if(trHashMsg == NULL) {
        status = tlsCpyHashEmptyInput(hash_id , &trHashRdy); // MUST NOT modify the hashed empty string from this function
        if(status < 0) { goto end_of_derive; }
    } else {
...
tlsRespStatus  tlsDeriveHStrafficSecret(tlsSession_t *session, tlsOpaque8b_t* earlysecret_in)
{
    ...
    sec->secret.hs.hs.len  = hash_sz; // must free up the space after master secret of the session is generated
    ...
    status = tlsHKDFextract( hash_algo_id, hash_sz, &sec->secret.hs.hs, &shared_secret, &derived_secret );
    if(status < 0) { goto end_of_derive; }
    // get TrHash(ClientHello...ServerHello)
    tlsOpaque8b_t  trHash_CHtoSH  = {derived_secret.len, derived_secret.data}; // reuse the allocated memory in derived_secret
    status = tlsTransHashTakeSnapshot(sec, hash_algo_id, trHash_CHtoSH.data, (word16) trHash_CHtoSH.len);
    if(status < 0) { goto end_of_derive; }
    // Derive-Secret(handshake secret, "c hs traffic", | TrHash(ClientHello...ServerHello)) = client_handshake_traffic_secret
    tlsOpaque8b_t  clientlabel = {12, (byte *)&("c hs traffic")};
    status = tlsDeriveSecret( hash_algo_id, &sec->secret.hs.hs, &clientlabel, &trHash_CHtoSH, &sec->secret.hs.client );
    if(status < 0) { goto end_of_derive; }
    // Derive-Secret(handshake secret, "s hs traffic", | TrHash(ClientHello...ServerHello)) = server_handshake_traffic_secret
    tlsOpaque8b_t  serverlabel = {12, (byte *)&("s hs traffic")};
    status = tlsDeriveSecret( hash_algo_id, &sec->secret.hs.hs, &serverlabel, &trHash_CHtoSH, &sec->secret.hs.server );
end_of_derive:
    if(earlysecret_in->data != NULL) {
...
tlsRespStatus  tlsDeriveAPPtrafficSecret(tlsSession_t *session)
{
    ...
    if(status < 0) { goto end_of_derive; }
    // Derive-Secret(master secret, "c ap traffic", | TrHash(ClientHello...server Finished)) = client_application_traffic_secret_0
    tmp.data = sec->hashed_hs_msg.snapshot_server_finished;
    tlsOpaque8b_t  clientlabel = {12, (byte *)&("c ap traffic")};
    status = tlsDeriveSecret( hash_id, &sec->secret.app.mst, &clientlabel, &tmp, &sec->secret.app.client );
    if(status < 0) { goto end_of_derive; }
    // Derive-Secret(master secret, "s ap traffic", | TrHash(ClientHello...server Finished)) = server_application_traffic_secret_0
    tlsOpaque8b_t  serverlabel = {12, (byte *)&("s ap traffic")};
    status = tlsDeriveSecret( hash_id, &sec->secret.app.mst, &serverlabel, &tmp, &sec->secret.app.server );
    if(status < 0) { goto end_of_derive; }
    // generate resumption master secret,
    // Derive-Secret(master secret, "res master", | TrHash(ClientHello...client Finished)) = resumption_master_secret
    tmp.data  = &derived_secret.data[hash_sz];
    ...
    status = tlsDeriveSecret( hash_id, &sec->secret.app.mst, &resumptionlabel, &tmp, &sec->secret.app.resumption );
end_of_derive:
    if(derived_secret.data != NULL) {
        XMEMFREE((void *)derived_secret.data);
        derived_secret.data = NULL;
...
tlsRespStatus  tlsDeriveTraffickey(tlsSecurityElements_t *sec, tlsOpaque8b_t  *in_rd_secret, tlsOpaque8b_t  *in_wr_secret)
{
    ...
    if((sec->chosen_ciphersuite == NULL) || (in_rd_secret->data == NULL) || (in_wr_secret->data == NULL)) {
        return TLS_RESP_ERRARGS;
    }
    ...
    if(status < 0) { goto end_of_gen_hs_key; }
...
static  tlsRespStatus  tlsActivateHSkeyHelper(tlsSecurityElements_t *sec, const tlsCipherSpec_t *cs, const byte isDecrypt)
{
    if((sec == NULL) || (cs == NULL)) {
        return TLS_RESP_ERRARGS;
    }
