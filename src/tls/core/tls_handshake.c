...
static void tlsCleanSpaceAfterGenHSkeys(tlsSession_t *session
    ...
    tlsFreeEphemeralKeyPairByGrp(
        session->sec.ephemeralkeyremote, session->sec.agreed_keyex_named_grp
    ); // ignore the return code
    tlsFreeEphemeralKeyPairByGrp(
        session->sec.ephemeralkeylocal, session->sec.agreed_keyex_named_grp
    ); // ignore the return code
...
void tlsHSstateTransition(tlsSession_t *session) {
    ...
    case TLS_HS_TYPE_FINISHED:
        if (session->flgs.hs_server_finish == 0) {
            ...
            session->flgs.outflight_flush =
                1; // TODO: find the better way to let application toggle this flag
        } else {
            if (session->flgs.new_session_tkt != 0) {
                session->hs_state = TLS_HS_TYPE_NEW_SESSION_TICKET;
            } else if (session->flgs.key_update != 0) {
                session->hs_state = TLS_HS_TYPE_KEY_UPDATE;
            }
        }
        break;
...
static tlsRespStatus tlsActivateHSsecrets(tlsSession_t *session) {
    ...
    status =
        tlsGenEarlySecret(session->sec.chosen_ciphersuite, session->sec.chosen_psk, &early_secret);
    if (status < 0) {
        goto done;
    }
    ...
    status = tlsDeriveTraffickey(
        &session->sec, &session->sec.secret.hs.server, &session->sec.secret.hs.client
    );
    // RFC 8466, section 5.3 Per-Record Nonce
    // The sequence numbers for reading / writing records is set to zero whenever read/write key is
    // updated
...
static tlsRespStatus tlsActivateAPPsecrets(tlsSession_t *session) {
    ...
    status = tlsDeriveTraffickey(
        &session->sec, &session->sec.secret.app.server, &session->sec.secret.app.client
    );
    // RFC 8466, section 5.3 Per-Record Nonce
    // The sequence numbers for reading / writing records is set to zero whenever read/write key is
    // updated
...
tlsRespStatus tlsClientStartHandshake(tlsSession_t *session) {
    ...
    do { // Step #1: hello messages for negotiating cryptography parameters
        ...
        // wait for reply from remote peer as soon as all fragment(s) of current TLS record message
        // were sent.
        do {
            ...
            if ((tlsChkFragStateInMsg(session) & TLS_RESP_FIRST_FRAG) == TLS_RESP_FIRST_FRAG) {
                if (session->flgs.hello_retry != 0) {
                    // re-init transcript hash only when we decoded first fragment of the
                    // HelloRetryRequest (include the case that there's only one fragment in the
                    // HelloRetryRequest)
    ...
    tlsCleanSpaceAfterKeyEx(session);
    status = tlsTransHashCleanUnsuedHashHandler(&session->sec
    ); // find appropriate time to clean up the space
    if (status < 0) {
        goto end_of_hs;
    }
    // Step #2:
    // * generate handshake traffic secret, then activate handshake read (decrypt) keys
    // * client must receive the last unencrypted record message "Change Cipher Spec" then turn on
    // flag hs_rx_encrypt
    ...
    // Step #3: From here, the client is supposed to receive more encrypted record messages from the
    // server
    ...
    **************************/
    ...
    // Step #5: the client is supposed to send finished message, or certificate message if required
    // by the peer
