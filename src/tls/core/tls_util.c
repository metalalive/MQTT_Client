...
// currently we use static array instead because number of supported versions will not grow that
// fast the size of this array MUST NOT exceeds TLS_MAX_BYTES_SUPPORTED_VERSIONS
...
// This implementation only supports signature schemes that are mandatory to implement e.g. PSS and
// PKCS1
...
tlsRespStatus tlsAlertTypeCvtToTlsResp(tlsAlertType in) {
    ...
    switch (in) {
    // TODO: need seperate response code, the peer will send TLS_ALERT_TYPE_CLOSE_NOTIFY even the
    // TLS connection is normally closed
    ...
    case TLS_ALERT_TYPE_INSUFFICIENT_SECURITY:
        out = TLS_RESP_ERR_NO_KEYEX_MTHD_AVAIL;
        break;
