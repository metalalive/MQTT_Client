...
int mqttEncodeProps(byte *buf, mqttProp_t *props) {
    ...
    for (curr_prop = props; curr_prop != NULL; curr_prop = curr_prop->next) {
        ...
        case MQTT_DATA_TYPE_STRING:
            len =
                mqttEncodeStr(buf, (const byte *)curr_prop->body.str.data, curr_prop->body.str.len);
            break;
        case MQTT_DATA_TYPE_STRING_PAIR:
            len = mqttEncodeStr(
                buf, (const byte *)curr_prop->body.strpair[0].data, curr_prop->body.strpair[0].len
            );
            if (buf != NULL) {
                buf += len;
            }
            total_len += len;
            len = mqttEncodeStr(
                buf, (const byte *)curr_prop->body.strpair[1].data, curr_prop->body.strpair[1].len
            );
            break;
        default:
            len = 0;
...
int mqttDecodeProps(byte *buf, mqttProp_t **props, word32 props_len) {
    ...
    while (props_len > 0) { // first byte of each property must represent the type
        ...
        case MQTT_DATA_TYPE_STRING_PAIR: {
            ...
            len = mqttDecodeStr(
                &buf[0], curr_prop->body.strpair[0].data, &curr_prop->body.strpair[0].len
            );
            ...
            len += mqttDecodeStr(
                &buf[len], curr_prop->body.strpair[1].data, &curr_prop->body.strpair[1].len
            );
            break;
        }
...
word16 mqttGetPktID(void) { // TODO: set up semaphore for multithreading case
...
int mqttEncodePktConnect(byte *tx_buf, word32 tx_buf_len, mqttConn_t *conn) {
    ...
    if ((conn->flgs.will_enable != 0) &&
        (conn->lwt_msg.topic.data == NULL || conn->lwt_msg.buff == NULL)) {
        return MQTT_RESP_ERRARGS;
    }
    ...
    curr_buf_pos +=
        mqttEncodeStr(curr_buf_pos, (const byte *)conn->client_id.data, conn->client_id.len);
    // copy data for last will testament
    if (conn->flgs.will_enable != 0) {
        mqttMsg_t *lwtmsg = &conn->lwt_msg;
        word32     lwt_props_len = lwtmsg->pkt_len_set.props_len;
        // TODO: figure out where to correctly append will properties in CONNECT packet for MQTT v5
        // protocol.
        curr_buf_pos += mqttEncodeVarBytes(curr_buf_pos, lwt_props_len);
        curr_buf_pos += mqttEncodeProps(curr_buf_pos, lwtmsg->props);
        // append will topic
        curr_buf_pos +=
            mqttEncodeStr(curr_buf_pos, (const byte *)lwtmsg->topic.data, lwtmsg->topic.len);
        // append will payload
        curr_buf_pos +=
            mqttEncodeStr(curr_buf_pos, (const byte *)lwtmsg->buff, lwtmsg->app_data_len);
    }
    if (conn->username.data != NULL) {
        curr_buf_pos +=
            mqttEncodeStr(curr_buf_pos, (const byte *)conn->username.data, conn->username.len);
    } else {
        // [Note]
    ...
    if (conn->password.data != NULL) {
        curr_buf_pos +=
            mqttEncodeStr(curr_buf_pos, (const byte *)conn->password.data, conn->password.len);
    }
...
int mqttDecodePktConnack(byte *rx_buf, word32 rx_buf_len, mqttPktHeadConnack_t *connack) {
    ...
    if (fx_head_len == 0) {
        return MQTT_RESP_ERR_CTRL_PKT_TYPE;
    }
    ...
    if (end_of_buf > curr_buf_pos) {
        ...
        if (autual_copied_len < 0) {
            return autual_copied_len;
        }
        curr_buf_pos += autual_copied_len;
    }
    if (curr_buf_pos != end_of_buf) {
        return MQTT_RESP_MALFORMED_DATA;
    }
...
int mqttEncodePktPublish(byte *tx_buf, word32 tx_buf_len, struct __mqttMsg *msg) {
    ...
    fx_head_len = mqttEncodeFxHeader(
        tx_buf, tx_buf_len, remain_len, MQTT_PACKET_TYPE_PUBLISH, msg->retain, msg->qos,
        msg->duplicate
    );
...
int mqttDecodePktPublish(byte *rx_buf, word32 rx_buf_len, struct __mqttMsg *msg) {
    ...
    fx_head_len = mqttDecodeFxHeader(
        rx_buf, rx_buf_len, &payload_len, MQTT_PACKET_TYPE_PUBLISH, &msg->retain, (byte *)&msg->qos,
        &msg->duplicate
    );
    if (fx_head_len == 0) {
        return MQTT_RESP_ERR_CTRL_PKT_TYPE;
    ...
    var_head_len += props_len;
    {
        ...
        if (curr_cp_len < 0) {
            return (mqttRespStatus)curr_cp_len;
        }
        if (curr_cp_len != props_len) {
            return MQTT_RESP_ERR_PROP;
        } // TODO: test
...
int mqttEncodePktSubscribe(byte *tx_buf, word32 tx_buf_len, mqttPktSubs_t *subs) {
    ...
    for (idx = 0; idx < subs->topic_cnt; idx++) {
        ...
        curr_buf_pos += mqttEncodeStr(
            curr_buf_pos, (const byte *)curr_topic->filter.data, curr_topic->filter.len
        );
        *curr_buf_pos =
            (byte)curr_topic->qos; // TODO: implement all fields of the subscription options byte
        curr_buf_pos++;
    }
    return (fx_head_len + remain_len);
...
int mqttEncodePktAuth(byte *tx_buf, word32 tx_buf_len, mqttAuth_t *auth) {
    ...
    fx_head_len =
        mqttEncodeFxHeader(tx_buf, tx_buf_len, remain_len, MQTT_PACKET_TYPE_AUTH, 0, 0, 0);
    curr_buf_pos = &tx_buf[fx_head_len];
    if (auth->reason_code != MQTT_REASON_SUCCESS) {
...
int mqttEncodePktPing(byte *tx_buf, word32 tx_buf_len) {
    ...
    word32 remain_len = 0;
    fx_head_len =
        mqttEncodeFxHeader(tx_buf, tx_buf_len, remain_len, MQTT_PACKET_TYPE_PINGREQ, 0, 0, 0);
    return (fx_head_len + remain_len);
} // end of mqttEncodePktPing

int mqttDecodePktPubResp(
    ...
    if (end_of_buf > curr_buf_pos) {
        ...
        if (autual_copied_len != props_len) {
            return MQTT_RESP_ERR_PROP;
        }
...
int mqttDecodePktSuback(byte *rx_buf, word32 rx_buf_len, mqttPktSuback_t *suback) {
    ...
    suback->return_codes = (byte *)XMALLOC(
        sizeof(byte) * reason_codes_len
    ); // TODO: find better way to allocate / free the space
...
int mqttDecodePktUnsuback(byte *rx_buf, word32 rx_buf_len, mqttPktUnsuback_t *unsuback) {
    ...
    reason_codes_len = (word16)(end_of_buf - curr_buf_pos);
    unsuback->return_codes = (byte *)XMALLOC(
        sizeof(byte) * reason_codes_len
    ); // TODO: find better way to allocate / free the space
...
int mqttDecodePktAuth(byte *rx_buf, word32 rx_buf_len, mqttAuth_t *auth) {
    ...
    byte  *end_of_buf;
    fx_head_len = mqttDecodeFxHeader(
        rx_buf, rx_buf_len, &remain_len, MQTT_PACKET_TYPE_AUTH, NULL, NULL, NULL
    );
    if (fx_head_len == 0) {
        return MQTT_RESP_ERR_CTRL_PKT_TYPE;
    }
...
int mqttDecodePktDisconn(byte *rx_buf, word32 rx_buf_len, mqttPktDisconn_t *disconn) {
    ...
    fx_head_len = mqttDecodeFxHeader(
        rx_buf, rx_buf_len, &remain_len, MQTT_PACKET_TYPE_DISCONNECT, &reservebits[0],
        &reservebits[1], &reservebits[2]
    );
    if (fx_head_len == 0) {
        return MQTT_RESP_ERR_CTRL_PKT_TYPE;
    }
    ...
    // reason code can be omitted if it is 0x00 (SUCCESS)
    disconn->reason_code =
        (end_of_buf > curr_buf_pos) ? *curr_buf_pos++ : MQTT_REASON_NORMAL_DISCONNECTION;
...
mqttPktRead(struct __mqttCtx *mctx, byte *buf, word32 buf_max_len, word32 *copied_len) {
    ...
    *copied_len = 0;
    // ----------- get fixed header -----------
    rd_len = mqttNetPktRecv(mctx, buf, 0x1);
    if (rd_len < 0) {
        return (mqttRespStatus)rd_len;
    }
    if (rd_len != 0x1) {
        return MQTT_RESP_MALFORMED_DATA;
    }
    ...
    *copied_len = rd_len;
    ...
    for (idx = 0; idx < MQTT_PKT_MAX_BYTES_REMAIN_LEN; idx++) {
        rd_len = mqttNetPktRecv(mctx, &buf[idx], 0x1);
        if (rd_len < 0) {
            return rd_len;
        }
        if (rd_len != 0x1) {
    ...
    *copied_len += idx + 1;
    buf_max_len -= idx + 1;
    // extract remaining length, the return of the following function below should be the same as
    // idx + 1
...
mqttRespStatus mqttDecodePkt(
    struct __mqttCtx *mctx, byte *buf, word32 buf_len, mqttCtrlPktType cmdtype, void **p_decode,
    word16 *recv_pkt_id
) {
    ...
    case MQTT_PACKET_TYPE_PUBCOMP: {
        ...
        *recv_pkt_id = (*(mqttPktPubResp_t **)p_decode)->packet_id;
        ...
        if ((cmdtype == MQTT_PACKET_TYPE_PUBRECV) || (cmdtype == MQTT_PACKET_TYPE_PUBREL)) {
            // if error is found in reason code & QoS = 2, then we abort subsequent packet
            // transmission (TODO: recheck this logic)
            status =
                mqttChkReasonCode((mqttReasonCode)(*(mqttPktPubResp_t **)p_decode)->reason_code);
            if (status < 0) {
                break;
            }
